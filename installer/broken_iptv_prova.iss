; Installer della COPIA DI PROVA (Windows).
;
; ⚠️ Generato a mano dal gemello stabile, ma le tre cose che contano sono
; diverse di proposito e NON vanno riallineate:
;   - AppId diverso: senza, l'installer scriverebbe sopra l'app vera invece di
;     affiancarsi (Inno riconosce un'installazione dal suo AppId);
;   - nome e cartella diversi, così le due copie non si mescolano;
;   - output BrokenIPTVProva.exe.
; I dati sono separati dal lato Dart (kDataSubdirectory in core/build_channel).
;
...
#define MyAppName "Broken IPTV Prova"
...
[Setup]
AppId={{7C4A9E21-5D8B-4F63-9A17-2E6B0D3F8C44}
AppName={#MyAppName}
...
OutputDir=output
OutputBaseFilename=BrokenIPTVProva
Compression=lzma
